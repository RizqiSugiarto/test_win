...
export class CreateProductController implements Controller {
    ...
    handle = async (request: CreateProductController.Request): Promise<HttpResponse<any>> => {
        console.log(request,"GINI")
        try {
            const createProduct = request;
            const requiredFields = createProduct.nameProduct && createProduct.price && createProduct.photoProduct && createProduct.description;
